...
var addAuthHeaders = function(settings, headers) {
    ...
    if (settings.userAgent) {
        ...
        if (settings.userAgentPassword) {
            ...
            var retsUaAuth = crypto.createHash('md5').update([a1, "", "", settings.version || headers['RETS-Version']].join(":")).digest('hex');
            headers['RETS-UA-Authorization'] = "Digest " + retsUaAuth;

            if (settings.delegateId && settings.delegateHash && settings.delegatePassword) {
                headers['X-Delegate-ID'] = settings.delegateId;
                var delegateAuth = crypto.createHash('md5').update([retsUaAuth, settings.delegatePassword, settings.delegateHash, settings.delegateId].join(":")).digest('hex');
                headers['X-Delegate-Authorization'] = "Digest " + delegateAuth;
            }
        }
    }
};
