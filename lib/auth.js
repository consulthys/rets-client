...
var addAuthHeaders = function(settings, headers) {
    ...
    // add basic auth, don't rely on request module
    if (settings.username && settings.password) {
        var userpass = [settings.username, settings.password].join(":");
        headers['Authorization'] = 'Basic ' + (new Buffer(userpass || '', 'ascii')).toString('base64');
    }
