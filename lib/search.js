...
var query = function(resourceType, classType, queryString, callback, options) {
    ...
    var _checkResults = function(error, searchResults) {
        if (error && callback) return callback(error, allResults);

        if (searchResults.count > 0) {
            allResults = allResults.concat(searchResults.data);
            if (allResults.length < searchResults.total) {
                logger.debug("Fetched %s of %s total records", allResults.length, searchResults.total);
                queryOpts.offset += searchResults.count - 1;
